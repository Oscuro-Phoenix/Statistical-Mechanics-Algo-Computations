...
while n_trials < 1000000:
    Vol_list = []
    Vol_list_squared = []
    for n_runs in range(0,10):
        delta = 0.1
        Q = 2
        d_max = 20
        ...
        Vol_list.append(Q)
        Vol_list_squared.append(Q**2)
    avg_sqr = sum(Vol_list_squared)/len(Vol_list_squared)
    sqr_avg = (sum(Vol_list)/len(Vol_list))**2
    stderr = math.sqrt(avg_sqr-sqr_avg)/math.sqrt(10)
    avg_Vol = math.sqrt(sqr_avg)
    true_Vol = V_sph(20)
    print(n_trials,'|',avg_Vol,'|',true_Vol,'|',stderr,'|',abs(avg_Vol-true_Vol))
